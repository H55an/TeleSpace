...
async def process_message_for_saving(message: Message) -> dict | None:
    """
    #[تعديل جوهري]: هذه الدالة تعالج الملفات والرسائل النصية وتجهزها للحفظ.
    """
    # الحالة 1: الرسالة تحتوي على ملف
    ...
    if file_type and file_obj:
        # بالنسبة للملفات، نعيد توجيهها للحصول على file_id دائم
        ...
        return {
            ...
            'item_type': file_type,
            'content': message.caption, # الوصف
            'file_unique_id': file_obj.file_unique_id,
            'file_id': fwd_file_obj.file_id
        }
        
    # الحالة 2: الرسالة هي نص فقط
    elif message.text:
        return {
            'item_name': f"رسالة: {message.text[:20]}...",
            'item_type': 'text',
            'content': message.text, # النص الكامل للرسالة
            'file_unique_id': None,
            'file_id': None
        }
    return None

async def send_items_paginated(update: Update, context: ContextTypes.DEFAULT_TYPE, folder_id: int, offset: int = 0):
    """
    #[تعديل جوهري]: هذه الدالة الآن تعيد إرسال النصوص وترسل الملفات بوصفها.
    """
    chat_id = update.effective_chat.id
    items_page, total_items = database.get_items_paginated(folder_id, limit=PAGE_SIZE, offset=offset)
    
    # ... (منطق التحقق من وجود عناصر يبقى كما هو)
    ...
    for item in items_page:
        try:
            ...
            if item_type == 'text':
                # إذا كان العنصر نصًا، نرسله كرسالة جديدة
                await context.bot.send_message(chat_id=chat_id, text=content)
            else:
                # إذا كان العنصر ملفًا، نرسله مع الوصف
                if item_type == 'document': await context.bot.send_document(chat_id=chat_id, document=file_id, caption=content)
                elif item_type == 'video': await context.bot.send_video(chat_id=chat_id, video=file_id, caption=content)
                elif item_type == 'photo': await context.bot.send_photo(chat_id=chat_id, photo=file_id, caption=content)
                elif item_type == 'audio': await context.bot.send_audio(chat_id=chat_id, audio=file_id, caption=content)
            
            await asyncio.sleep(0.5)
        except Exception as e:
            await context.bot.send_message(chat_id, f"لم يتم إرسال العنصر '{item['item_name']}'. الخطأ: {e}")
            
    # ... (منطق زر "التالي" يبقى كما هو)
    new_offset = offset + len(items_page)
    if new_offset < total_items:
        keyboard = [[InlineKeyboardButton(f"📥 إرسال الـ {min(PAGE_SIZE, total_items - new_offset)} عناصر التالية", callback_data=f"view_files:{folder_id}:{new_offset}")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await context.bot.send_message(chat_id, "لا يزال هناك المزيد من العناصر.", reply_markup=reply_markup)
    else:
        await context.bot.send_message(chat_id, "تم إرسال كل العناصر في هذا المجلد.")


# ... (دوال start, button_press_router, ومحادثات الإنشاء تبقى كما هي)
...
async def button_press_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("section:"):
        ...
        await send_items_paginated(update, context, int(folder_id), int(offset))
    elif data == "back_to_main":
        await start(update, context)

# ... (Conversation handlers for section and folder creation remain the same)
...
async def collect_files_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if message_text and message_text.lower() == "حفظ":
        ...
        if not message_buffer:
            await update.message.reply_text("لم يتم إرسال أي عناصر للحفظ. تم إلغاء العملية.")
            context.user_data.clear(); await start(update, context); return ConversationHandler.END
        await update.message.reply_text(f"جاري حفظ {len(message_buffer)} عنصر... يرجى الانتظار.")
        count = 0
        ...
        await update.message.reply_text("👍 تم الاستلام. أرسل المزيد أو أرسل `حفظ` للإنهاء.")
        return AWAITING_FILES_FOR_UPLOAD
