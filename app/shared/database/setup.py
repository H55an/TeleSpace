...
def setup_database():
    """
    يتصل بقاعدة بيانات PostgreSQL، ينشئ الجداول بالهيكلية الأحدث،
    وينفذ التحديثات (Migrations) للجداول الموجودة مسبقاً.
    """
    conn = None
    try:
        ...
        print("Setting up tables with the latest schema...")

        # --- Users Table ---
        # تم دمج الأعمدة الجديدة هنا للنسخ الجديدة من قاعدة البيانات
        cursor.execute("""
        ...
        # --- Items Table ---
        # تم دمج بيانات الملفات الإضافية هنا
        cursor.execute("""
        ...
        # ==========================================
        # المزامنة والتحديثات للجداول القديمة (Migrations)
        # ==========================================
        print("Running migrations for existing tables (if any)...")
        ...
        print(f"An error occurred during database setup/migration: {e}")
        traceback.print_exc()
        if conn:
